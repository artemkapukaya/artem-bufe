...
import {
    MapPin, Package, User, ChevronRight, LogOut,
    X, Trash2, Edit2, Plus, Mail, ShoppingBag, Phone, Building2, ShieldCheck
} from "lucide-react";
...
export default function ProfilePage() {
    ...
    const [vault, setVault] = useState<any>({
        user: {
            ...
            email: "ergin@artembufe.com",
            phone: "+90 531 958 80 88",
            regDate: "24 Nisan 2026",
        },
        addresses: [],
    });

    // Vault'u localStorage'dan yükle
    useEffect(() => {
        const saved = localStorage.getItem("artem_user_vault");
        if (saved) {
            try { setVault(JSON.parse(saved)); } catch { }
        }
    }, []);

    // Siparişleri yükle — sekme değişimi + 5sn polling + yeni sipariş gelince anlık
    useEffect(() => {
        const loadOrders = () => {
            try {
    ...
    const handleSaveAddress = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        setVault((prev: any) => ({
            ...prev,
            addresses: editingAddr
                ? prev.addresses.map((a: any) => a.id === editingAddr.id ? newAddr : a)
    ...
    const handleDeleteAddress = (id: string) => {
        if (confirm("Bu adresi silmek istediğinize emin misiniz?")) {
            setVault((prev: any) => ({
                ...prev,
                addresses: prev.addresses.filter((a: any) => a.id !== id)
            }));
            toast.error("Adres silindi.");
        }
    };
    ...
    const clearAllOrders = () => {
        if (!window.confirm("Tüm siparişlerinizi silmek istediğinize emin misiniz?")) return;
        setRealOrders([]);
        localStorage.removeItem("artem_orders");
        toast.success("Tüm siparişler temizlendi.");
    };

    // Artem Gem hesaplama — realOrders'a bağlı, component body'sinde
    ...
    return (
        <div className="min-h-screen bg-[#F8F9FA] flex flex-col pt-20 font-sans text-gray-800">
            ...
            <main className="flex-1 max-w-[1200px] mx-auto w-full p-4 md:p-8">
                <div className="flex flex-col lg:flex-row gap-8 mt-6">
                    <aside className="w-full lg:w-72 shrink-0">
                        <div className="bg-white rounded-[32px] p-5 shadow-sm border border-gray-100 sticky top-28">
                            <div className="px-4 py-6 text-center border-b border-gray-50 mb-4">
                                <div className="w-20 h-20 bg-red-50 rounded-full flex items-center justify-center mx-auto mb-3 text-red-500 shadow-inner">
                                    <User size={40} />
                                </div>
                            ...
                            <nav className="space-y-1">
                                <SideTab icon={<Package size={18} />} label="Siparişlerim" active={activeTab === "orders"} onClick={() => setActiveTab("orders")}
                                    badge={realOrders.length} />
                                <SideTab icon={<User size={18} />} label="Hesap Bilgilerim" active={activeTab === "account"} onClick={() => setActiveTab("account")} />
                                <SideTab icon={<span className="text-base">💎</span>} label="Artem Gem" active={activeTab === "coins"} onClick={() => setActiveTab("coins")}
                                    badge={totalGems > 0 ? totalGems : 0} />
                    ...
                    <div className="flex-1">
                                ...
                                <div className="flex items-center justify-between ml-2 mb-2">
                                    ...
                                    {realOrders.length > 0 && (
                                        <button
                                            ...
                                            className="flex items-center gap-1.5 text-xs font-bold text-red-400 hover:text-red-600 hover:bg-red-50 px-3 py-1.5 rounded-xl transition-all border border-red-100 hover:border-red-300"
                                        >
                                ...
                                {realOrders.length > 0 ? realOrders.map((order: any) => (
                                    <div key={order.id} className="bg-white rounded-[32px] border border-gray-100 shadow-sm overflow-hidden group hover:border-red-500 transition-all duration-300">
                                        <div className="p-4 bg-gray-50/50 border-b border-gray-100 flex items-center justify-between gap-2">
                                            <span className="text-[11px] font-black uppercase text-gray-400 truncate">
                                                #{order.id} | {order.date}
                                            </span>
                                            <div className="flex items-center gap-2 shrink-0">
                                                <span className={`text-[10px] font-black px-3 py-1.5 rounded-full uppercase ${statusColor[order.status] ?? "bg-orange-100 text-orange-600"}`}>
                                                    {order.status}
                                                </span>
                                                <button
                                                    onClick={() => deleteOrder(order.id)}
                                                    title="Siparişi Sil"
                                                    className="w-7 h-7 flex items-center justify-center rounded-lg bg-red-50 hover:bg-red-100 text-red-400 hover:text-red-600 transition-all border border-red-100"
                                                >
                                                    <Trash2 size={13} />
                                                </button>
                                            </div>
                                        ...
                                        <div className="p-6 space-y-3">
                                            {order.images?.map((item: any, i: number) => (
                                        ...
                                        <div className="px-6 pb-5 flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-t border-gray-50 pt-4">
                                            <p className="text-xs text-gray-400 flex items-center gap-1.5"><MapPin size={12} className="text-orange-400" /> {order.address}</p>
                                            <p className="text-3xl font-black text-red-600 tracking-tighter">{order.total}</p>
                                        </div>
                                    </div>
                                )) : (
                        ...
                        {activeTab === "coins" && (
                            <div className="space-y-6 animate-in fade-in slide-in-from-right-4">
                                ...
                                <div className="bg-white rounded-[28px] border border-gray-100 shadow-sm p-6">
                                    <h4 className="font-black text-gray-800 mb-4">Nasıl Gem Kazanırsın?</h4>
                                    <div className="space-y-3">
                                        {[
                                            { icon: "🛒", text: "Her 100 TL harcamada 5 Gem kazanırsın", color: "bg-orange-50 border-orange-100" },
                                            { icon: "📸", text: "Instagram'da paylaşım yapınca 20 Gem bonus", color: "bg-pink-50 border-pink-100" },
                                            { icon: "🎰", text: "Şans çarkı kazanmalarında 10 Gem bonus", color: "bg-purple-50 border-purple-100" },
                                        ].map(({ icon, text, color }) => (
                                            <div key={text} className={`flex items-center gap-3 p-3 rounded-2xl border ${color}`}>
                                ...
                                {realOrders.length > 0 && (
                                    <div className="bg-white rounded-[28px] border border-gray-100 shadow-sm p-6">
                                        <h4 className="font-black text-gray-800 mb-4">Gem Kazanım Geçmişi</h4>
                                        <div className="space-y-2">
                                            {realOrders.slice(0, 5).map((o: any) => {
                                                const amount = parseFloat(o.total?.replace(/[^0-9,]/g, "").replace(",", ".")) || 0;
                                                const earned = Math.floor(amount / 100) * 5;
                                                return earned > 0 ? (
                                                    <div key={o.id} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
                                                        <div>
                                                            <p className="text-xs text-gray-400 font-bold">#{o.id}</p>
                                                            <p className="text-sm font-semibold text-gray-700">{o.total}</p>
                                                        </div>
                                                        <span className="font-black text-amber-500">+{earned} 💎</span>
                                                    </div>
                                                ) : null;
                                            })}
                                        </div>
                        ...
                        {activeTab === "account" && (
                            <div className="bg-white rounded-[32px] p-10 border border-gray-100 shadow-sm animate-in fade-in">
                                <h3 className="font-bold text-xl mb-8 border-b pb-4">Profil Bilgileri</h3>
                                <div className="grid md:grid-cols-2 gap-6">
                                    <ProfileField label="Ad Soyad" value={vault.user.fullName} onChange={(v: string) => setVault({ ...vault, user: { ...vault.user, fullName: v } })} />
                                    <ProfileField label="E-Posta" icon={<Mail size={14} />} value={vault.user.email} onChange={(v: string) => setVault({ ...vault, user: { ...vault.user, email: v } })} />
                                ...
                                <div className="mt-10 flex flex-col md:flex-row items-center justify-between gap-6 border-t pt-8">
                                    <button onClick={handleUpdateProfile} className="bg-red-600 text-white font-black uppercase text-[11px] py-5 px-12 rounded-2xl hover:bg-black transition-all shadow-xl">Kaydet</button>
                                    <div className="flex items-center gap-4">
                                        <button onClick={() => setShowKvkkModal(true)} className="flex items-center gap-2 text-gray-400 font-bold text-[10px] uppercase tracking-widest hover:text-blue-500 border-r pr-4 border-gray-100">
                                            <ShieldCheck size={14} /> KVKK Metni
                                        </button>
                                        <button onClick={() => confirm("Hesabınızı silmek istediğinize emin misiniz?")} className="flex items-center gap-2 text-emerald-500 font-black text-[10px] uppercase hover:text-emerald-600 transition-all">
                                    ...
                                    {vault.addresses.map((addr: any) => (
                                        <div key={addr.id} className="bg-white p-6 rounded-[28px] border border-gray-100 flex justify-between items-center group hover:border-red-500 transition-all">
                                            <div className="flex gap-5 items-center">
                                                <div className="p-4 bg-gray-50 rounded-2xl text-gray-400 group-hover:text-red-500"><MapPin size={24} /></div>
                                                <div>
                                                    <div className="flex items-center gap-2 mb-1">
                                                        <h4 className="font-bold text-sm uppercase">{addr.title}</h4>
                                                        <span className={`text-[9px] px-2 py-0.5 rounded font-black uppercase ${addr.type === 'Kurumsal' ? 'bg-blue-50 text-blue-600' : 'bg-gray-100 text-gray-500'}`}>{addr.type}</span>
                                                    </div>
                                                    <p className="text-xs text-gray-400">{addr.detail}</p>
                                                    <div className="flex gap-3 mt-2 font-bold text-[9px] text-gray-300 uppercase">
                                                        <span className="flex items-center gap-1"><Phone size={10} /> {addr.phone}</span>
                                                        {addr.type === "Kurumsal" && <span className="text-blue-400">{addr.taxOffice} / {addr.taxNumber}</span>}
                                                    </div>
                                                </div>
                                            </div>
                                            <div className="flex gap-2">
                                                ...
                                                <button onClick={() => handleDeleteAddress(addr.id)} className="p-3 bg-gray-50 text-gray-400 rounded-xl hover:text-red-600"><Trash2 size={18} /></button>
                                            </div>
                                        </div>
                                    ))}
                                </div>
            ...
            {/* KVKK & ADRES MODALLARI BURAYA GELECEK (KODUNUZDAKİYLE AYNI) */}
            {/* ... Modal kodları ... */}
            {showKvkkModal && (
                <div className="fixed inset-0 bg-black/70 backdrop-blur-md z-[3000] flex items-center justify-center p-4">
                    <div className="bg-white rounded-[40px] w-full max-w-2xl p-10 max-h-[80vh] flex flex-col shadow-2xl animate-in zoom-in-95">
                        <div className="flex justify-between items-center mb-6 border-b pb-4">
                            <h3 className="text-xl font-black uppercase">KVKK Aydınlatma Metni</h3>
                            <button onClick={() => setShowKvkkModal(false)}><X className="text-gray-400" /></button>
                        </div>
                        <div className="flex-1 overflow-y-auto text-[13px] leading-relaxed text-gray-500 space-y-4 pr-2 italic">
                            <p><strong>Artem Büfe</strong> olarak verilerinizi 6698 sayılı Kanun kapsamında; sipariş yönetimi, teslimat ve ödeme süreçleri için güvenle işliyoruz. Bilgileriniz yasal zorunluluklar dışında 3. şahıslarla paylaşılmaz.</p>
                        </div>
                        <button onClick={() => setShowKvkkModal(false)} className="mt-8 bg-black text-white py-4 rounded-2xl font-black uppercase text-[10px] tracking-widest">OKUDUM</button>
                    </div>
                </div>
            )}

            {showAddrModal && (
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[2000] flex items-center justify-center p-4 animate-in fade-in">
                    <form onSubmit={handleSaveAddress} className="bg-white rounded-[40px] w-full max-w-lg p-10 shadow-2xl">
                        <div className="flex justify-between items-center mb-8"><h3 className="text-xl font-bold">{editingAddr ? 'Adresi Düzenle' : 'Yeni Adres'}</h3><button type="button" onClick={() => setShowAddrModal(false)}><X className="text-gray-300" /></button></div>
                        <div className="space-y-4">
                            ...
                            <input required name="title" defaultValue={editingAddr?.title} placeholder="Adres Başlığı (Örn: Ev, İş)" className="w-full bg-gray-50 border rounded-2xl px-6 py-4 font-bold outline-none focus:border-red-500 shadow-inner" />
                            <input required name="phone" defaultValue={editingAddr?.phone || vault.user.phone} placeholder="İletişim Numarası" className="w-full bg-gray-50 border rounded-2xl px-6 py-4 font-bold outline-none focus:border-red-500 shadow-inner" />
                            {addrType === "Kurumsal" && (
                                <div className="grid grid-cols-2 gap-4">
                                    <input required name="taxOffice" defaultValue={editingAddr?.taxOffice} placeholder="Vergi Dairesi" className="bg-gray-50 border rounded-2xl px-5 py-4 font-bold outline-none focus:border-red-500 shadow-inner" />
                                    <input required name="taxNumber" defaultValue={editingAddr?.taxNumber} placeholder="Vergi No" className="bg-gray-50 border rounded-2xl px-5 py-4 font-bold outline-none focus:border-red-500 shadow-inner" />
                                </div>
                            )}
                            <textarea required name="detail" defaultValue={editingAddr?.detail} placeholder="Mahalle, Cadde, Sokak, No, İl/İlçe..." className="w-full bg-gray-50 border rounded-2xl px-6 py-4 font-bold outline-none h-32 focus:border-red-500 resize-none shadow-inner" />
...
// SideTab ve ProfileField bileşenleri (aynı kalabilir)
    ...
    return (
        <button onClick={onClick} className={`w-full flex items-center justify-between p-4 rounded-2xl transition-all duration-300 ${active ? 'bg-red-50 text-red-600 shadow-sm border border-red-100' : 'text-gray-500 hover:bg-gray-50'}`}>
            ...
            <div className="flex items-center gap-2">
                {badge > 0 && <span className="bg-red-500 text-white text-[9px] font-black rounded-full min-w-[16px] h-4 flex items-center justify-center px-1">{badge}</span>}
...
function ProfileField({ label, value, onChange, readOnly, icon }: any) {
    return (
        <div className="space-y-2">
            <label className="text-[10px] font-black text-gray-400 uppercase ml-1 flex items-center gap-2">{icon} {label}</label>
            <input readOnly={readOnly} value={value} onChange={(e) => onChange && onChange(e.target.value)} className={`w-full bg-gray-50 border border-gray-100 rounded-[22px] px-6 py-4 font-bold text-sm outline-none transition-all ${readOnly ? 'opacity-50' : 'focus:border-red-500 shadow-inner'}`} />
        </div>
    );
}